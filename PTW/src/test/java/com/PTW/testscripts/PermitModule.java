...
//		cm.getplanenddate().sendKeys(afterNextDate);
	...
	/**	cm.getsecondarylocation().click();		//noted
		...
		testCase = extentReport.createTest("\"Isolation Type\" field.");
		testCase.log(Status.INFO, "Select none on the isolation type field.");  
		testCase.log(Status.PASS, " None option is selected.");
		System.out.println("None option is selected");
		Thread.sleep(2000);
		cm.getembedded().click();
		testCase = extentReport.createTest("\"Embeded Selection\" field.");
		testCase.log(Status.INFO, "Select Embeded selection type field.");  
		testCase.log(Status.PASS, " Embeded selection Appears.");
		System.out.println("Embeded selection Appears");
		Thread.sleep(2000);
